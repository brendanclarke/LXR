 ...
 * PresetLoadCache.c
 *
 * Transitional load/session cache ownership for the front-panel parser.
 * This file keeps the parser-local background-load bridge together while the
 * receive-side split is still in progress.
 */
...
static void presetLoad_applyDeferredPerfMessages(void)
{
   uint8_t i;
