...
| Current sample import and imported-sample playback | `knowledge_files/log_archive/030_SESSION_HANDOFF_LOG.md`, `knowledge_files/comms_spec_reference/COMMS_FLOW_SPEC.md` |
...
- Session 030 sample import uses STM32 internal flash sectors 8-11 only. PCM writes must stay below `SAMPLE_INFO_START_ADDRESS`, and the committed sample count at `SAMPLE_ROM_START_ADDRESS` is written last.
...
- Do not wait for a raw ACK during sample import. AVR must keep parsing UART traffic until `SAMPLE_UPLOAD_RESULT` latches completion, while progress packets update the LCD.
- Import order is `/samples` one-shots first, optional `/loops` loops second. Invalid, empty, odd-length, too-large, or unreadable files must not increment `loadedSamples` or create silent waveform slots.
